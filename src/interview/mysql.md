...
### 数据库的三范式 
- 第一范式（1NF）要求表中的每个字段都是原子性的，即不可再拆分。
- 第二范式（2NF）要求表中的非主键字段必须完全依赖于主键，而不能只依赖于主键的一部分。
- 第三范式（3NF）要求表中的非主键字段之间不能存在传递依赖关系，即非主键字段不能依赖于其他
- 好处
  - 遵循三范式可以减少数据冗余，建立结构合理的数据库，从而提高数据存储和使用的性能。
...
1. 快照读：
   ...
   - 当执行select的时候，innodb默认会执行快照读，相当于就是给你目前的状态照了一张照片，以后执行select 的时候就会返回当前照片里面的数据，
...
- 结论
    ...
    - 要完全避免，需要手动加锁（参考：快照读和当前读）将快照读调整为当前读（mysql不会自动加锁），然后mysql使用next-key完全避免了幻读，
...
1.功能对比
    ...
    - InnoDB支持ACID的事务4个特性，而MyISAM不支持；
      - 原子性（Atomicity）：事务是一个不可分割的工作单位，要么全部执行成功，要么全部执行失败。如果在事务执行过程中发生了错误，那么所有的操作都会被回滚，数据库会恢复到事务执行前的状态。 
      - 一致性（Consistency）：事务执行前后，数据库的状态应该保持一致。如果事务执行成功，那么数据库的状态应该符合预期；如果事务执行失败，那么数据库的状态应该回滚到事务执行前的状态。
      - 隔离性（Isolation）：事务之间应该相互隔离，互不干扰。每个事务应该感觉不到其他事务的存在，就好像它是唯一的事务一样。
      - 持久性（Durability）：事务执行成功后，对数据库的修改应该是永久性的。即使系统崩溃或者发生其他故障，数据库也应该能够恢复到事务执行成功后的状态。
...
- 优化
  ...
  - inner join延迟关联
    - 改为：SELECT acct1.id,acct1.name,acct1.balance  FROM  account  acct1  INNER  JOIN  
